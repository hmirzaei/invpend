...
static long theQueue[MAX_SIZE];// The queue
...
long Dequeue()
{
  ...
  if(isEmpty()) return '\0';  // Return null character if queue is empty
    ...
    {
      head++;
      data = theQueue[head % MAX_SIZE];// Get character to return
      return data;// Return popped character
    }
}
